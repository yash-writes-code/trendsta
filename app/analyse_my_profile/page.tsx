...
import { 
  ...
  Filter, Mic, Calendar, DollarSign, Loader2
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    if (!isPending && !session) {
      router.push('/signup?next=' + encodeURIComponent('/analyse_my_profile'));
    }
  ...
  useEffect(() => {
    ...
    const startPolling = () => {
      const poll = async () => {
        try {
          const res = await fetch('/api/analyse_my_profile', { cache: 'no-store' });
          if (res.status === 202) return; // not ready yet, keep polling
          if (res.ok) {
            const result = await res.json();
            const analysisPayload = result.analysis ?? result;
            setData(Array.isArray(analysisPayload) ? analysisPayload[0] : analysisPayload);
            if (pollRef.current) {
          ...
          console.error('Error polling analysis data:', err);
        }
      };
    ...
    const trigger = async () => {
      try {
        ...
        if (result.ready && result.analysis) {
          // Analysis already existed in DB — show it immediately, no polling needed
        ...
        // Fall back to polling in case of network error
        startPolling();
      }
    };
  ...
  // Typing effect for the AI terminal
  useEffect(() => {
    if (!data) return;
    ...
    return (
      <div className="min-h-screen bg-[#fafafa] flex flex-col items-center justify-center font-sans">
        ...
        <h2 className="text-xl font-bold font-serif-brand text-gray-900">
          {isPending ? 'Verifying Session...' : 'Preparing Your Analysis...'}
        </h2>
        <p className="text-gray-500 text-sm mt-2 font-medium">
          {isPending ? 'Please wait...' : 'Your report is being generated — this page will update automatically.'}
        </p>
      </div>
    );
  }
  ...
  const s2 = segments[2]?.percentage || 25;
              ...
              <ul className="space-y-4">
                <li className="flex justify-between items-center text-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-gray-700 font-medium">Reach Efficiency</span>
                  </div>
                ...
                <li className="flex justify-between items-center text-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-gray-700 font-medium">WPM Consistency</span>
                  </div>
                ...
                <li className="flex justify-between items-center text-sm">
                  <div className="flex items-center gap-3">
                    <Info className="w-4 h-4 text-orange-500" /> 
                    <span className="text-gray-700 font-medium">Snippet Duration</span>
                  </div>
                ...
                <li className="flex justify-between items-center text-sm">
                  <div className="flex items-center gap-3">
                    <Check className="w-4 h-4 text-gray-400" /> 
                    <span className="text-gray-700 font-medium">Ad Immunity</span>
                  </div>
        ...
        <main className="flex-1 w-full max-w-4xl mx-auto flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
            ...
            <div className="bg-gray-50 border border-gray-100 rounded-xl p-6 relative overflow-hidden">
              ...
              <h3 className="text-center text-2xl font-bold text-gray-900 font-serif-brand mb-2">
                Follower Utilization is too low.
              </h3>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100 relative overflow-hidden">
                ...
                <h4 className="flex items-center gap-2 text-gray-800 font-bold mb-4 font-serif-brand text-lg">
                    ...
                    <li key={idx} className="flex items-start gap-3 text-gray-600 text-sm leading-relaxed">
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${item.impact_level === 'High' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
              ...
              <div className="bg-orange-50/50 rounded-2xl p-6 border border-orange-100 relative overflow-hidden">
                  ...
                  {data.main_analysis.llm_diagnostic.critical_flaws.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-gray-600 text-sm leading-relaxed">
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-center mb-1">
                          <strong className="text-gray-800">{item.title}</strong>
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${item.impact_level === 'High' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}`}>
                            {item.impact_level} Impact
                          </span>
                        </div>
                        {item.description}
                ...
                <ActionableFixList 
                  clearFixes={[
                    "Stop placing CTAs in your descriptions. Transition to direct verbal hooks."
                  ]}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Performance Distribution Funnel */}
            <div className={"bg-white rounded-2xl p-6 shadow-sm border border-gray-200 relative group " + hoverClasses}>
              <h2 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-6 flex items-center gap-2">
                <Filter className="w-4 h-4 text-orange-500" /> Performance Funnel
              </h2>
              
              <div className="space-y-3 mt-4">
                {/* Missing Viral Tier */}
                <div className="relative">
                  <div className="flex justify-between text-[10px] font-bold uppercase mb-1 px-1">
                    <span className="text-orange-500 flex items-center gap-1"><Lock className="w-3 h-3"/> Missing Viral Tier</span>
                    <span className="text-gray-400">{data.main_analysis.performance_funnel.viral_percentage}%</span>
                  </div>
                  <div className="w-full h-8 bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center">
                    <span className="text-xs text-gray-400 font-medium italic">Target: 5% of content</span>
                  </div>
                </div>

                {/* Above Average */}
                <div className="relative pt-2">
                  <div className="mx-auto h-8 bg-orange-500 rounded-lg flex items-center justify-between px-4 text-white font-bold text-xs shadow-sm" style={{ width: (data.main_analysis.performance_funnel.above_average_percentage + 30) + '%' }}>
                    <span>Above Average</span>
                    <span>{data.main_analysis.performance_funnel.above_average_percentage}%</span>
                  </div>
                </div>

                {/* Average */}
                <div className="relative">
                  <div className="mx-auto h-8 bg-orange-300 rounded-lg flex items-center justify-between px-4 text-gray-900 font-bold text-xs" style={{ width: (data.main_analysis.performance_funnel.average_percentage + 55) + '%' }}>
                    <span>Average</span>
                    <span>{data.main_analysis.performance_funnel.average_percentage}%</span>
                  </div>
                </div>

                {/* Underperforming */}
                <div className="relative">
                  <div className="mx-auto h-8 bg-gray-200 rounded-lg flex items-center justify-between px-4 text-gray-600 font-bold text-xs" style={{ width: (data.main_analysis.performance_funnel.underperforming_percentage + 40) + '%' }}>
                    <span>Under</span>
                    <span>{data.main_analysis.performance_funnel.underperforming_percentage}%</span>
                  </div>
                </div>
              </div>
              ...
              <ActionableFixList 
                clearFixes={[
                  "Consistently post top segment content to increase your above average baseline."
                ]}
                blurredFix="Unlock Viral Formulas to see exactly how to hit the missing 5% target tier."
              />
            </div>
            ...
            <div className={"bg-white rounded-2xl p-6 shadow-sm border border-gray-200 group relative overflow-hidden " + hoverClasses}>
                ...
                {data.main_analysis.topic_performance_ladder.topics.map((item, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3">
                      <span className="w-20 text-[10px] font-bold text-gray-600 truncate">{item.topic_label}</span>
                      <div className="flex-1 h-2.5 bg-gray-100 rounded-full overflow-hidden">
                        <div className={"h-full rounded-full " + (colors[i] || colors[0])} style={{ width: item.percentile.value + '%' }}></div>
                      </div>
                      <span className="w-8 text-[10px] font-bold text-gray-900 text-right">{item.multiplier_score}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <ActionableFixList 
                clearFixes={[
                  "Double down on top ranking topics to sustain the high reach multiplier."
                ]}
            ...
            <div className={"bg-white rounded-2xl p-6 shadow-sm border border-gray-200 group relative overflow-hidden " + hoverClasses}>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="space-y-3 w-full">
                  {data.main_analysis.audience_engagement_map.segments.map((seg, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between text-xs font-bold text-gray-600">
                        <span className="flex items-center gap-2">
                          <span className={"w-2 h-2 rounded-full " + (colors[i] || 'bg-gray-200')}></span> {seg.segment_name}
                        </span>
              ...
              <ActionableFixList 
                clearFixes={[
                  "Prioritize content geared towards top 2 audience segments which drive the most engagement."
                ]}
              ...
              <div className="flex flex-col gap-3 mb-4">
                <div className="p-3 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <span className="text-lg font-black text-gray-900 font-serif-brand">{data.main_analysis.hook_power_analysis.top_hook.score}/100</span>
                  </div>
                </div>
                <div className="p-3 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-between opacity-70">
                  ...
                  <div className="text-right">
                    <span className="text-lg font-black text-gray-500 font-serif-brand">{data.main_analysis.hook_power_analysis.worst_hook.score}/100</span>
                  </div>
                </div>
                {hookNudge && (
                  <p className="text-xs font-bold text-orange-600 text-center bg-orange-50 py-1 rounded-lg">{hookNudge}</p>
                )}
              </div>

              <ActionableFixList 
                clearFixes={[
                  "Avoid the Q&A specific interview hook structures like your lowest performing hook."
                ]}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="relative h-16 w-full flex items-center justify-center mb-4 mt-4">
                  ...
                  <div className="w-full h-6 bg-gradient-to-r from-gray-100 via-orange-100 to-gray-100 rounded-full relative border border-gray-200">
                    <div className="absolute left-[50%] w-[25%] h-12 -top-3 bg-white border-2 border-orange-500 rounded-lg flex items-center justify-center shadow-md z-10 translate-x-[-12%]">
                      <span className="text-[10px] font-black text-gray-900 leading-tight text-center">
                        {data.main_analysis.wpm_consistency_band.min_wpm}-{data.main_analysis.wpm_consistency_band.max_wpm}<br/>WPM
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="absolute -bottom-6 w-full flex justify-between text-[10px] font-bold text-gray-400">
                    <span>150 WPM (Slow)</span>
                    <span>300 WPM (Fast)</span>
                  </div>
                </div>
            ...
            <div className={"bg-white rounded-2xl p-6 shadow-sm border border-gray-200 relative group " + hoverClasses}>
              ...
              <div className="h-20 flex items-end gap-1 px-4 border-b border-gray-200 mb-2 relative">
                ...
                <div className="w-1/3 flex justify-center relative group/deadzone">
                ...
                <div className="w-1/3 flex justify-center">
                  <div className="w-8 h-16 bg-gray-800 rounded-t-sm"></div>
                </div>
              </div>
              ...
              <ActionableFixList 
                clearFixes={[
                  `Push content past ${data.main_analysis.duration_sweet_spot.optimal_zones[1]} as those durations out-perform.`
                ]}
          ...
          {/* SECTION 6: MONETIZATION & AI Scripts */}
          <div className="mb-8">
