...
import { TRENDING_TOPICS, TWEETS, LATEST_TWEETS_FEED } from "../data/mockData";
...
function ScoreBadge({ score }: { score: number }) {
    const getLevel = (s: number) => {
        if (s >= 0.5) return { label: "Viral", className: "score-high" };
        if (s >= 0.1) return { label: "Hot", className: "score-medium" };
...
// Trending Topic Item
function TrendingItem({ topic, index }: { topic: typeof TRENDING_TOPICS[0]; index: number }) {
    return (
        <div
            className="flex items-center gap-4 p-4 hover:bg-blue-50 rounded-xl transition-all duration-200 cursor-pointer group"
            style={{ animationDelay: `${index * 0.05}s` }}
        >
            <span className="text-2xl font-bold text-slate-300 w-8 group-hover:text-blue-400 transition-colors">
                #{topic.rank}
            </span>
        ...
        >
            ...
            <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                        <span className="font-semibold text-slate-900 truncate">{tweet.author}</span>
...
function LatestTweetItem({ tweet, index }: { tweet: typeof LATEST_TWEETS_FEED[0]; index: number }) {
    return (
        <div
            className="p-4 hover:bg-blue-50 rounded-xl transition-all duration-200 border-b border-slate-100 last:border-b-0"
            ...
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                        <span className="font-semibold text-slate-900 text-sm">{tweet.author}</span>
                        <span className="text-xs text-slate-500">{tweet.handle}</span>
                        <span className="text-xs text-slate-400">• {tweet.postedAgo}</span>
                    </div>
                    <p className="text-sm text-slate-600 line-clamp-2">{tweet.content}</p>
                ...
                <div className="max-w-7xl mx-auto space-y-8">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fadeInUp" style={{ animationDelay: '0.1s' }}>
                        <div className="stat-card">
                            <p className="text-2xl font-bold text-slate-900">{TWEETS.length}</p>
                            <p className="text-xs text-slate-500">Top Tweets</p>
                        </div>
                        <div className="stat-card">
                            <p className="text-2xl font-bold text-emerald-600">53.8K</p>
                            <p className="text-xs text-slate-500">Max Views</p>
                        </div>
                        <div className="stat-card">
                            <p className="text-2xl font-bold text-slate-900">342K</p>
                            <p className="text-xs text-slate-500">Top Followers</p>
                        </div>
                        <div className="stat-card">
                            <p className="text-2xl font-bold text-slate-900">1.28</p>
                            ...
                            {activeTab === "top" ? (
                                ...
                                <div className="glass-card overflow-hidden">
                                    ...
                                    <div className="stagger-children">
                                        {LATEST_TWEETS_FEED.map((tweet, index) => (
